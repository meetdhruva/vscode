...
suite('ExtHostWebview', () => {
	...
	test('asWebviewUri for remote with / and + in name', () => {
		...
		assert.strictEqual(
			webviewUri.toString(),
			`https://vscode-remote%2Bssh-002dremote-002blocalhost-003dfoo-002fbar.vscode-resource.${webviewResourceBaseHost}/Users/cody/x.png`,
			'Check transform');

		assert.strictEqual(
			decodeAuthority(webviewUri.authority),
			`vscode-remote+${authority}.vscode-resource.${webviewResourceBaseHost}`,
	...
	test('asWebviewUri for remote with port in name', () => {
		...
		assert.strictEqual(
			webviewUri.toString(),
			`https://vscode-remote%2Blocalhost-003a8080.vscode-resource.${webviewResourceBaseHost}/Users/cody/x.png`,
			'Check transform');
