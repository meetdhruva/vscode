...
- Replace Microsoft-owned signing and release credentials.
  The inherited VS Code release pipeline files still reference Microsoft ESRP, Azure subscriptions, Key Vaults, publisher names, release owners, distro mixins, and VS Code release metadata. Aster does not use those as the public-download path. Aster needs GitHub-hosted release workflows plus its own Windows code-signing certificate, Apple Developer Team ID/certificates/profiles/notarization credentials, Linux package signing keys, publishing storage, release approvers, distro source or replacement plan, service connections, and a configured `docs/aster-release-infrastructure.json` before signed installers can be produced. Use [Aster Release Infrastructure Inputs](./aster-release-infrastructure-inputs.md) as the implementation checklist.

- Configure public download publishing.
  Public installable downloads should be served from GitHub Release assets, following the VSCodium-style release asset pattern. GitHub Actions workflows build or download GitHub-hosted artifacts, create or reuse the configured GitHub Release, upload installables, and attach `.sha1` and `.sha256` sidecars. See [Aster Public Downloads](./aster-public-downloads.md). This is not a substitute for signing, package repositories, update metadata, or the release-infrastructure evidence manifest.
