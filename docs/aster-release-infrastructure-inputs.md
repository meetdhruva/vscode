...
- Release storage buckets or object containers for installers, archives, symbols, checksums, update metadata, and webview preload assets. The first public download surface is GitHub Release assets, configured through GitHub Actions workflow inputs and the secret `ASTER_GITHUB_RELEASE_TOKEN` when the default `GITHUB_TOKEN` is not sufficient.
...
`npm run aster:check-release-readiness` scans Aster GitHub workflows and release scripts for inherited Microsoft-owned release infrastructure, including:
...
The check is expected to fail until these references are replaced or the inherited pipeline is removed from the Aster release surface. A passing source scan is necessary but not sufficient for a public release; GitHub Actions publishing builds also call `npm run aster:check-release-artifacts -- <unpacked artifact paths...>` against unpacked desktop, server, web, and extension artifacts before artifact publication.

## Pipeline Gate

Aster public downloads are GitHub Actions only. The inherited Azure product pipeline defaults release publishing off for Aster; `build/azure-pipelines/product-build.yml` requires both `VSCODE_PUBLISH: true` and `ASTER_RELEASE_INFRA_CONFIRMED: true` before the inherited publish stage can be selected. `build/azure-pipelines/product-publish.yml` and `build/azure-pipelines/product-release.yml` also fail early when invoked directly without `ASTER_RELEASE_INFRA_CONFIRMED: true`.
...
- `ASTER_GITHUB_RELEASE_TOKEN`, when the GitHub workflow cannot use the default `GITHUB_TOKEN`
